...
export default function Quiz() {
  ...
  const getInterpretation = () => {
    ...
    if (total <= 20) return { range: "Mild (11–20) — Some indicators", html: <><p>Some features consistent with central sensitization. Moderate signal — worth exploring.</p><p>Start with the <Link href="/intro/what-is" className="text-teal hover:underline">intro guides</Link> and <Link href="/intro/science" className="text-teal hover:underline">science page</Link>. If the evidence resonates, explore <Link href="/protocols/prt" className="text-teal hover:underline">PRT techniques</Link>.</p></> };
    if (total <= 30) return { range: "Moderate (21–30) — Significant indicators", html: <><p>Significant pattern of central sensitization. Neuroplastic mechanisms are likely playing an important role.</p><p>Dive into the <Link href="/protocols/prt" className="text-teal hover:underline font-semibold">PRT protocol</Link> or <Link href="/protocols/schubiner" className="text-teal hover:underline font-semibold">Schubiner&apos;s method</Link>. The landmark PRT trial showed 66% pain-free rates.</p></> };
    return { range: "High (31–40) — Strong indicators", html: <><p>Strongly suggests central sensitization — common in fibromyalgia, chronic widespread pain, and related syndromes. Well-studied and highly treatable.</p><p><strong>You&apos;re in the right place.</strong> Start with the <Link href="/intro/what-is" className="text-teal hover:underline">intro guide</Link>, then work through the <Link href="/protocols/prt" className="text-teal hover:underline">PRT protocol</Link>. Consider finding a <Link href="/resources" className="text-teal hover:underline">PRT-trained practitioner</Link>. The JAMA trial showed 66% recovery in 4 weeks.</p></> };
  };
  ...
  if (done) {
    ...
    return (
      <div className="max-w-[720px] mx-auto px-6 py-16">
        ...
        <div className="rounded-xl p-5 bg-teal-light/70 dark:bg-teal/10 border border-transparent mb-6">
          ...
          <ul className="list-disc pl-5 m-0 text-sm text-text-muted space-y-1.5">
            <li>Read the <Link href="/intro/what-is" className="text-teal font-semibold hover:underline">intro guides</Link></li>
            <li>Explore <Link href="/protocols/prt" className="text-teal font-semibold hover:underline">Pain Reprocessing Therapy</Link></li>
            <li>Dive into <Link href="/protocols/schubiner" className="text-teal font-semibold hover:underline">Schubiner&apos;s method</Link></li>
            <li>Check the <Link href="/faq" className="text-teal font-semibold hover:underline">FAQ</Link></li>
            <li>Browse <Link href="/resources" className="text-teal font-semibold hover:underline">books, courses, and practitioners</Link></li>
            <li>Discuss central sensitization with a PRT-trained healthcare provider</li>
          </ul>
        </div>

        <div className="flex gap-3 flex-wrap">
          <button onClick={restart} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-teal text-white font-semibold hover:bg-teal-dark transition">Restart quiz</button>
          <Link href="/intro/what-is" className="inline-flex items-center px-5 py-2.5 rounded-lg border-2 border-teal text-teal font-semibold hover:bg-teal-light transition">Start the intro guides</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-[720px] mx-auto px-6 py-16">
      ...
      <div className="mb-6">
        ...
        <div className="bg-border dark:bg-zinc-700 h-1.5 rounded-full">
          <div className="bg-teal h-1.5 rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
        </div>
      </div>
          ...
          {q.options.map((opt, i) => (
            <label
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 cursor-pointer transition-all text-text dark:text-zinc-100 ${
                answers[current] === i
                  ? "border-teal bg-teal-light dark:bg-teal/20"
                  : "border-border dark:border-zinc-700 hover:border-teal-light hover:bg-teal-light/30 dark:hover:bg-teal/10"
              }`}
            >
              <input
                type="radio"
      ...
      <div className="flex gap-3">
        {current > 0 && (
          <button
            ...
            className="inline-flex items-center px-4 py-2 rounded-lg border-2 border-teal text-teal font-semibold hover:bg-teal-light transition"
          >
            &larr; Back
          </button>
        )}
        {answers[current] !== null && current === questions.length - 1 && (
          <button
            ...
            className="inline-flex items-center px-5 py-2 rounded-lg bg-teal text-white font-semibold hover:bg-teal-dark transition"
          >
            See results &rarr;
          </button>
        )}
